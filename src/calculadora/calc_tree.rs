use super::{CalcPriority, CalcResult, InputOption};
pub struct CalcTree {
    valor: InputOption,
...
impl CalcTree {
    pub fn create_from(operaciones: &[InputOption], vec_priority: &[CalcPriority]) -> Option<Self> {
        ...
        loop {
            if operaciones.len() == 0 {
                break;
            }
            if vec_priority[i] == priority_check {
                let mut arbol = CalcTree::new(operaciones[i].clone());
                arbol.children.as_mut().0 =
                    Self::create_from(&operaciones[..i], &vec_priority[..i]);
                arbol.children.as_mut().1 =
                    Self::create_from(&operaciones[i + 1..], &vec_priority[i + 1..]);
    ...
    pub fn operacion(&self) -> CalcResult {
        match &self.valor {
            ...
            InputOption::Operator(o) => {
                let Some(hijo_1) = &self.children.as_ref().1 else {
                    return CalcResult::SyntaxError;
                };
                match o.as_str() {
                    "+" => {
                        let Some(hijo_0) = &self.children.as_ref().0 else {
                            return CalcResult::SyntaxError;
                        };
